        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="w-full md:w-1/2">
                    <form action="{{ route("dashboard.applications") }}" method="GET">   
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-4 py-4">Application</th>
                            <th scope="col" class="px-4 py-3">Advert</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$application->id}}</th>
                            <td class="px-4 py-3">{{$application->advertTitle}}</td>
                            <td class="px-4 py-3">{{$application->partnerName}}</td>
                            <td class="px-4 py-3">{{$application->status}}</td>
                            <td class="px-4 py-3">{{$application->created_at->format('M d, Y H:i')}}</td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                <button id="{{ $application->id }}-dropdown-button" data-dropdown-toggle="{{ $application->id }}-dropdown" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                    </svg>
                                </button>
                                <div id="{{ $application->id }}-dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm" aria-labelledby="{{ $application->id }}-dropdown-button">
                                        <li>
                                            <button data-id="{{$application->id}}" type="button" data-modal-target="deleteModal" data-modal-toggle="deleteModal" class="deleteBtn flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
                                                Delete
                                            </button>
                                        </li>
                                    </ul>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-center items-center space-x-4">
                <form action="{{ route('application.destroy', '__ID__') }}" method="POST" id="form">
                    @csrf
                    @method('DELETE')
