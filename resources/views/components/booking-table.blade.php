                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$category->category_name}}</th>
                            <td class="px-4 py-3 flex items-center justify-end">
                                ...
                                <div id="{{ $category->id }}-dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm" aria-labelledby="{{ $category->id }}-dropdown-button">
                                        <li>
                                            <button data-id="{{$category->id}}" type="button" data-modal-target="updateProductModal" data-modal-toggle="updateProductModal" class="editBtn flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                        ...
                                        <li>
                                            <button data-id="{{$category->id}}" type="button"           data-modal-target="deleteModal" 
...
<div id="updateProductModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <!-- Modal body -->
            <x-spinner />
            <form action="{{route("category.update", "__ID__")}}" method="POST" id="editcategoryForm">
                @csrf
                @method('PUT')
                ...
                <div class="flex items-center space-x-4">
                    <button type="submit" class="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Update category</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-center items-center space-x-4">
                <form action="{{ route('category.destroy', '__ID__') }}" method="POST" id="form">
                    @csrf
                    @method('DELETE')
